...
⚠️★ **以上这条主线已被 [#70](stage4-findings.md) 推翻**：改成 `bus_instance=1`
之后传感器**根本注册不出来**，保持 5 才能注册出 UID —— 说明**总线是通的、
芯片被探到了、出厂配置没错**。`irq_trigger_type`（出厂 1，改 3 就注册不上）
与 `rail_on_state`（改不改都一样）也一并洗清。

**现在的定性**：能探到、能注册、**就是不出数** —— 典型的数据就绪中断
（`dri_irq_num` = TLMM 127）没到。

**第一步（改过的）**：换观测手段，别再改配置。
① 让 AP 临时把 127 申请成中断源数边沿（会和 SSC 抢，风险未知）；
② 抄一台光感在主线上能用的 sc8280xp（ThinkPad X13s）；
③ 看 SEE 有没有轮询模式 —— 能绕开 DRI 的话既验证了假说又顺手把功能做了。

★ 本轮还留下一个可复用的**免构建实验回路**（私有 `hexagonrpcd -R` 根目录
+ 每格必跑的加速度计对照），见 #70 开头。
